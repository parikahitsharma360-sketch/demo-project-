...
exports.getAllcategory = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ sucess: true, data: category });

    } catch (err) {

        res.status(201).json({ sucess: false, message: err.message });
    }
};
...
exports.getUpdatecategory = async (req, res) => {
    ...
    try {
        ...
        if (result.rowaffected === 0) {
            ...
            return res.status(404).json({ sucess: false, message: 'category not updated' })
        }
        ...
        res.status({ sucess: true, message: 'category updated sucessfully' })


    } catch (err) {

        res.status(404).json({ sucess: false, message: err.message })
    }
};
...
exports.deleteCategory = async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.query('DELETE FROM category WHERE category_id = ? ?'[req.params.body]);
        ...
        res.staus(500).json({
            ...
            message: 'category deleted unsucessfully',
